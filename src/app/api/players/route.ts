...
export async function GET(request: NextRequest) {
  try {
    ...
    // FIX: Remove `rank` from ORDER BY since the column doesn't exist
    // Get players
...
export async function POST(request: NextRequest) {
  try {
    ...
    // FIX: Also check if rank column exists before inserting
    // First, check if rank column exists
    const columnCheck = await query("SHOW COLUMNS FROM players LIKE 'rank'");
    const hasRankColumn = Array.isArray(columnCheck) && columnCheck.length > 0;
    
    if (hasRankColumn) {
      // If rank column exists, include it
      const result = await query(
        ...
        [
          ...
          data.rank || 0,
          data.points || 0,
          ...(hasRankColumn ? [data.rank || 0] : [])
        ]
      );
    } else {
      // If rank column doesn't exist, don't include it
      const result = await query(
        `INSERT INTO players (
          name, age, position, avatar_url, team, attendance, 
          discipline_score, points, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
        [
          data.name,
          data.age,
